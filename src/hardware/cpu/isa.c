...
{
    //在调用函数时才能知道寄存器具体地址是多少，所以只能放到函数里面
    reg_t *reg = &(cr->reg);
...
//"mov    %rsp,%rbp"
    ...
    //printf(" instruction src  ==> %s \n", src_str);
    //printf(" instruction dst  ==> %s \n", dst_str);
...
*/
    ...
    else if (strcmp(op_str, "leave") == 0 || strcmp(op_str, "leaveq") == 0)
    {
        inst->op = INST_LEAVE;
    }
    ...
    else if (strcmp(op_str, "ret") == 0 || strcmp(op_str, "retq") == 0)
    {
        inst->op = INST_RET;
    }
    ...
    else if (strcmp(op_str, "cmp") == 0 || strcmp(op_str, "cmpq") == 0)
    {
        inst->op = INST_CMP;
    }
...
static void sub_handler(od_t *src_od, od_t *dst_od, core_t *cr)
{
    ...
    if (src_od->type == IMM && dst_od->type == REG)
    {
        ...
        cr->flags.ZF = (val == 0);
        cr->flags.SF = val_sign;
        cr->flags.OF = ((src_sign == 1 && dst_sign == 0 &&
                        val_sign == 1) || (src_sign == 0 && 
                        dst_sign == 1 && val_sign == 0));
        // update registers
        *(uint64_t *)dst = val;
        // signed and unsigned value follow the same addition. e.g.
